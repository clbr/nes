...
int main(int argc, char **argv) {
	...
	if (argc < 3)
		die("Usage: %s coord tiles.png [out.png]\n", argv[0]);
	...
	for (y = 0; y < h; y++) {
		for (x = 0; x < w; x++) {
			...
			const u16 sy = tile / (inw / 8);
			const u16 sx = tile % (inw / 8);
			const u8 * const from = &tiles[sy * 8 * inw + sx * 8];
			u8 * const to = &out[y * 8 * 8 * w + x * 8];
			for (ty = 0; ty < 8; ty++) {
				memcpy(to + ty * 8 * w, from + ty * inw, 8);
			}
		}
	}
